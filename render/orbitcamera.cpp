...
QVector3D OrbitCamera::position() const {
    ...
    //球坐标to笛卡尔坐标
    return m_target + QVector3D(m_distance * std::cos(pitch) * std::cos(yaw), m_distance * std::sin(-pitch),
                                m_distance * std::cos(pitch) * std::sin(yaw));
}

QMatrix4x4 OrbitCamera::viewMatrix() const {
    QMatrix4x4 view;
    view.lookAt(position(), m_target, {0.0f, 1.0f, 0.0f});
    return view;
}
...
//平移观察目标，不是沿世界坐标 X/Y，而是屏幕左右、上下方向移动。
...
void OrbitCamera::zoom(float wheelSteps) {
    m_distance = std::clamp(m_distance * std::pow(0.85f, wheelSteps), 1.0f, 50.0f);
}

void OrbitCamera::reset() {
    m_target = {0.0f, 1.2f, 0.0f};
    m_yawDegrees = -42.0f;
    m_pitchDegrees = -22.0f;
    m_distance = 7.0f;
}

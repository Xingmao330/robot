                   ...
                   const QVector3D &maximum, float *distance)
{
    ...
    for (int axis = 0; axis < 3; ++axis) {
        ...
        nearDistance = std::max(nearDistance, first); farDistance = std::min(farDistance, second);
                          ...
                          const QVector<QMatrix4x4> &nodeTransforms)
{
    ...
    bool invertible = false; const QMatrix4x4 inverseVP = (projection * view).inverted(&invertible);
    ...
    float closestDistance = std::numeric_limits<float>::max(); int closestNode = -1;
    const QVector<GlbNode> &nodes = model.nodes();
    for (int nodeIndex = 0; nodeIndex < nodes.size() && nodeIndex < nodeTransforms.size(); ++nodeIndex) {
        bool invertibleModel = false; const QMatrix4x4 inverseModel = nodeTransforms[nodeIndex].inverted(&invertibleModel);
        ...
        for (const int meshIndex : nodes[nodeIndex].meshIndices) {
            ...
            float distance = 0.0f; const GlbMesh &mesh = model.meshes()[meshIndex];
            if (intersectsBox(localOrigin, localDirection, mesh.minimum, mesh.maximum, &distance) && distance < closestDistance) { closestDistance = distance; closestNode = nodeIndex; }
        }
    }
    return closestNode;
}
